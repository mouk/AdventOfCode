...
impl Cube {
    ...
    fn new_from_line(line: &str) -> (Self, bool) {
        ...
        let add = ins == "on";
        assert!(add || ins == "off");
        let ((xa, xp), (ya, yp), (za,zp)) = line.split(",").map(to_tuple).collect_tuple().unwrap();
        assert_eq!(xa,"x");
        assert_eq!(ya,"y");
        assert_eq!(a,"z");
     ...
     */
    fn new_from_tuples(add: bool, xp: (i32, i32), yp: (i32, i32), zp: (i32, i32)) -> Self {
