...
lines = """dcagfb cbegda cabdf bagedf dcb cd dcbegaf aebcf bgdaf dgcf | dacebgf cbafd dbacf cdgf
...
signals = list(map(lambda l: l.split(' | ')[0], lines))
displays = list(map(lambda l: l.split(' | ')[1], lines))

freq = ['abcefg','cf','acdeg','acdfg','bcdf','abdfg','abdefg','acf','abcdfeg','abcdfg']
freq_list = list(map(lambda x: ''.join(sorted(x)),freq))

freq_map = {value:index  for (index, value) in enumerate(freq_list)}
print('freq_list', freq_list)

c = Counter(''.join(freq))
freq_dict = {}
for l,f in c.items():
    freq_dict[f]= (freq_dict.get(f) or [] ) +[l]

print('freq_dict', freq_dict)
pre_sel = {len(value):list(value)  for (index, value) in enumerate(freq_list) if index in [1, 4, 7, 8 ]}
print('pre_sel', pre_sel)


def get_line_sum(the_index):
    l1 = signals[the_index]
    dict = {}
    for c in l1.split(' '):
        if len(c) in pre_sel.keys():
            for l in c:
                dict[l] = (dict.get(l) or []) + [pre_sel[len(c)]]
    c = Counter(''.join(l1.split(' ')))
    print('', c)
    ...
    def disjoint(lists):
        ...
        for index in range(1,len(lists)):
            results = [i for i in results if i in lists[index]]
        return results
    ...
    for (l,freq) in c.items():
        statistical_freq = freq_dict.get(freq)
        if statistical_freq:
            dict[l] = (dict.get(l) or []) + [statistical_freq]

    print('dict', dict.items())
    first_cleaned = [(key, disjoint(val)) for (key,val) in dict.items()]

    print('first_cleaned', first_cleaned)


    found = list(map(lambda l : l[1][0],filter(lambda p: len(p[1]) == 1, first_cleaned)))
    print('found', found)

    second_cleaned = {key:  (val if len(val) == 1 else [i for i in val if i not in found])[0]  for (key,val) in first_cleaned}
    print('second_cleaned', second_cleaned)
    vals = displays[the_index].split(' ')
    s = list(map(lambda one_val: freq_map[''.join(sorted(list(map(lambda letter:second_cleaned[letter], one_val))))], vals))
    s = [value * (10**index) for (index, value) in enumerate(reversed(s)) ]
    print('vals', s)
    return sum(s)

print('sum', sum(map(get_line_sum, range(len(signals)))))